...
class SendRequest(LoginRequiredMixin, View):
	def get(self, request, id):
		...
		frequest, created = FriendRequest.objects.get_or_create(from_user=self.request.user,to_user=user)
		return HttpResponseRedirect('/users/{}'.format(user.profile.slug))

class CancelRequest(LoginRequiredMixin, View):
	def get(self, request, id):
		user = get_object_or_404(User, id=id)
		frequest = FriendRequest.objects.filter(from_user=self.request.user,to_user=user).first()
		frequest.delete()
		return HttpResponseRedirect('/users/{}'.format(user.profile.slug))
...
class AcceptRequest(LoginRequiredMixin,View):
	def get(self, request, id):
		...
		if(FriendRequest.objects.filter(from_user=request.user, to_user=from_user).first()):
			request_rev = FriendRequest.objects.filter(from_user=self.request.user, to_user=from_user).first()
			request_rev.delete()
		frequest.delete()
...
class My_Profille(View):
	def get(self, request, slug):
		...
		if p not in self.request.user.profile.friends.all():
			button_status = 'not_friend'
			if len(FriendRequest.objects.filter(from_user=self.request.user).filter(to_user=you)) == 1:
				button_status = 'friend_request_sent'
			if len(FriendRequest.objects.filter(from_user=p.user).filter(to_user=self.request.user)) == 1:
				button_status = 'friend_request_received'
